...
class RobotFeedbackSenstivity(Enum):
    ...
    def conn_type(senstivity):
        if senstivity == None:
            ...
            raise ValueError(
                "Value has to be of enum RobotFeedbackSenstivity. Provided {} Type {}".format(
                    senstivity, type(senstivity)
                )
            )
        return conn_type
...
class RobotInitialNetwork:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.s_local_senstivity = RobotFeedbackSenstivity.conn_type(
            kwargs.pop("s_local_senstivity", None)
        )
        self.s_rostl_senstivity = RobotFeedbackSenstivity.conn_type(
            kwargs.pop("s_rostl_senstivity", None)
        )
        self.s_caudl_senstivity = RobotFeedbackSenstivity.conn_type(
            kwargs.pop("s_caudl_senstivity", None)
        )

        self.reaction_local = kwargs.pop("reaction_local", None)
        self.reaction_local_senstivity = kwargs.pop(
            "reaction_local_senstivity", "REACTION2FREQ"
        )
