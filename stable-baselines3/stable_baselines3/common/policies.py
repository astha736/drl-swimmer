...
class ActorCriticPolicy(BasePolicy):
    ...
    def _get_action_dist_from_latent(self, latent_pi: th.Tensor) -> Distribution:
        ...
        # in the case of localFeedback latent_pi already containt mean of actions
        if conf.CONF["RL"]["localFeedback"]:
            mean_actions = latent_pi  # self.action_net(latent_pi)
        else:
    ...
    def evaluate_actions(
        ...
        if self.share_features_extractor:
            ...
            latent_vf = self.mlp_extractor.forward_critic(vf_features)
        distribution = self._get_action_dist_from_latent(latent_pi)
        log_prob = distribution.log_prob(actions)  # shape (batch_size)
        values = self.value_net(latent_vf)
