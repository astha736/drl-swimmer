        ...
        for i in range(n_envs):
            if episode_counts[i] < episode_count_targets[i]:
                ...
                if dones[i]:
                    ...
                    current_lengths[i] = 0
                    if custom_metrics:
                        # if env.venv doesnt exist, probably no normWrapper is used
                        try:
                            env_metrics = env.venv.envs[0].env.metrics
                        except:    
                            env_metrics = env.envs[0].env.metrics
                        for metric in env_metrics:
                            if not metric in metrics:
                                metrics[metric] = []
                            metrics[metric].append(env_metrics[metric])
